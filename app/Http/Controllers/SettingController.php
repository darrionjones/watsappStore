    ...
    public function saveBusinessSettings(Request $request)
    {
        ...
        if(\Auth::user()->type == 'super admin')
        {
            ...
            if($request->light_logo)
            {
                ...
                //  Settings::updateOrCreate(['name' =>  'light_logo', 'created_by' =>  \Auth::user()->creatorId(),'store_id' => 0],['value' => $logoName]);
            }
            ...
            if($request->favicon)
            {
                ...
                // Settings::updateOrCreate(['name' =>  'favicon', 'created_by' =>  \Auth::user()->creatorId(),'store_id' => 0],['value' => $favicon]);
        ...
        { 
            ...
            if($request->company_light_logo)
            {
                ...
                // $path     = $request->file('light_logo')->storeAs('uploads/logo/', $logoName);
                $company_logo = !empty($request->company_light_logo) ? $logoName : 'logo-light.png';
            ...
            if(!empty($request->title_text) || !empty($request->cust_darklayout) || !empty($request->cust_theme_bg) || !empty($request->color))
            {
                // $post = $request->all();
                ...
                //     if (in_array($key, array_keys($settings))) {
                //         \DB::insert(
                //             'insert into settings (`value`, `name`,`created_by`,`store_id`) values (?, ?, ?, ?) ON DUPLICATE KEY UPDATE `value` = VALUES(`value`) ,`type` = VALUES(`type`)', [
                //                 $data,
                //                 $key,
                //                 $user->current_store,
                //                 \Auth::user()->type,
